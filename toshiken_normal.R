...
# データ読み込み
data <- read.xlsx('home_death.xlsx', sheet="R_hospital_clinic")
...
hospital_ratio <- data$hospital_ratio
clinic <- data$clinic
clinic_ratio <- data$clinic_ratio
...
# 変数（列名）命名
colnames(data_toshiken) <- c("year", "prefecture_code", "city_code", "population", "population_over65", "elderly_ratio", "taxable_income", "per_capita_taxable_income", "hospital", "hospital_ratio", "clinic", "clinic_ratio", "nursing_station", "nursing_station_ratio", "home_death", "year2015", "year2016", "year2017")
...
# hospital, clinicを数値扱いに
data_toshiken$hospital <- as.integer(data_toshiken$hospital)
head(data_toshiken$hospital)
data_toshiken$clinic <- as.integer(data_toshiken$clinic)
head(data_toshiken$clinic)
...
# 被説明変数と説明変数
all_variables <- cbind(year, elderly_ratio, per_capita_taxable_income, hospital_ratio, clinic_ratio, nursing_station_ratio, home_death, year2015, year2016, year2017)

# 説明変数
explanatory_variables <- cbind(year, city_code, elderly_ratio, per_capita_taxable_income, hospital_ratio, clinic_ratio, nursing_station_ratio, year2015, year2016, year2017)
...
result1 = plm(home_death ~ elderly_ratio + per_capita_taxable_income + hospital_ratio + clinic_ratio + nursing_station_ratio + year2015 + year2016 + year2017, data=data_refine, model="pooling")
summary(result1)
...
#GLS推定（変量効果推定）
result3 = plm(home_death ~ elderly_ratio + per_capita_taxable_income + hospital_ratio + clinic_ratio + nursing_station, data=data_refine, model="random")
summary(result3)
#行列式の計算がうまくいっていない
#逆行列が求まらないから？
...
#有意水準p<0.05とすると、この結果は統計的に有意。帰無仮説は棄却→固定効果推定
